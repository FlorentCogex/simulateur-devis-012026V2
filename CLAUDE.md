...
- `COGEX_Lettre_Rupture.html` — a termination letter opened via "📤 Lettre de Résiliation (côté client)", written **in the client's own voice, to their current/outgoing accountant** — COGEX must never be named or branded anywhere in this file (no logo, no `Lucie/Florent DESBLANCS` header, neutral serif styling instead of the green COGEX theme) since the client is the ostensible author. Same `{{PLACEHOLDER}}` pattern, keyed under `rupture_*` (`rupture_client_nom/adresse/siret/forme_capital`, `rupture_cabinet_nom/adresse`, `rupture_lieu_date`, `rupture_civilite`, `rupture_date_effet`, `rupture_clause_reference` — a pre-composed optional clause, empty string when no lettre-de-mission article reference was given — `rupture_signataire_nom/qualite`, `rupture_motifs` as a JSON array of strings rendered as a `<ul>` only when non-empty). Preserve the "COGEX never appears" constraint in any future edit to this file.
